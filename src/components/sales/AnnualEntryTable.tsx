...
import { AnnualSalesRowData } from "@/hooks/useAnnualSalesEntry";

interface AnnualEntryTableProps {
  entries: AnnualSalesRowData[];
  totals: {
    quantite: number;
    ca: number;
    by_channel: {
...
export function AnnualEntryTable({ entries, totals, isLoading, mode, onUpdate }: AnnualEntryTableProps) {
  ...
  const handleSave = (entry: AnnualSalesRowData) => {
    ...
    if (editingCell.field === 'quantite') {
      ...
      onUpdate({
        product_id: editingCell.productId,
        categorie_prix: editingCell.channel,
        quantite_annuelle: channelData.quantite_annuelle,
  ...
  const handleKeyDown = (e: React.KeyboardEvent, entry: AnnualSalesRowData) => {
  ...
  if (entries.length === 0) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>Saisie annuelle - {mode === 'budget' ? 'Budget' : 'Réel'}</CardTitle>
          <CardDescription>
          ...
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="flex items-center gap-2">
                Saisie annuelle
                <Badge variant={mode === 'budget' ? 'default' : 'secondary'}>
                  {mode === 'budget' ? 'Budget' : 'Réel'}
                </Badge>
              </CardTitle>
              <CardDescription>
                Une ligne = 1 produit × 3 canaux. La quantité annuelle sera répartie automatiquement sur les 12 mois.
              </CardDescription>
            </div>
          ...
          <Table>
            <TableHeader>
              ...
              <TableRow>
                {CHANNELS.map(ch => (
                  <>
                    <TableHead key={`${ch}-qty`} className="text-right text-xs border-l">Qté/an</TableHead>
                    ...
                    return (
                      <>
                              ...
                              <TooltipTrigger asChild>
                                <button
                                  className={cn(
                                    ...
                                    channelData.quantite_annuelle === 0 && "text-muted-foreground"
                                  )}
                                  onClick={() => handleEdit(entry.product_id, ch, 'quantite', channelData.quantite_annuelle)}
                                >
                                  {channelData.quantite_annuelle}
                        ...
                        <TableCell key={`${entry.product_id}-${ch}-ca`} className="text-right font-medium">
                          {channelData.ca_annuel > 0 ? formatCurrency(channelData.ca_annuel) : '-'}
                        </TableCell>
                      </>
                    );
                  })}
